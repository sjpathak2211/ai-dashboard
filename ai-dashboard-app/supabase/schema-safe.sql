-- Supabase Schema for AI Progress Dashboard (Safe Version)
-- This version checks for existing objects before creating them

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Drop existing types if they exist (for clean installation)
DO $$ 
BEGIN
  -- Check and create request_status type
  ...
  -- Check and create request_priority type
  ...
  -- Check and create department_type
  ...
  -- Check and create backlog_status_enum (renamed to avoid conflict)
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'backlog_status_enum') THEN
    CREATE TYPE backlog_status_enum AS ENUM ('clear', 'busy', 'swamped');
  END IF;

  -- Check and create activity_type
...
-- Insert initial backlog status (only if not exists)
