-- Phase 2 Migration: Fix Database Schema Issues
-- This script fixes RLS policies and ensures proper user creation
...
-- Create AI Requests table (if not exists)
...
-- Create Backlog Status table (if not exists)
CREATE TABLE IF NOT EXISTS public.backlog_status (
...
CREATE INDEX IF NOT EXISTS idx_activity_log_request_id ON public.activity_log(request_id);
...
-- Insert initial backlog status (if not exists)
...
-- CRITICAL FIX: Drop all existing RLS policies first
ALTER TABLE public.users DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.ai_requests DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.activity_log DISABLE ROW LEVEL SECURITY;
ALTER TABLE public.backlog_status DISABLE ROW LEVEL SECURITY;
...
-- Users policies with FIXED INSERT policy
...
-- CRITICAL: Allow system to insert activities
...
BEGIN
  ...
  VALUES (
    ...
    CASE
      WHEN NEW.email IN ('shyam@ascendcohealth.com', 'shyam.pathak@ascendcohealth.com', 'admin@ascendcohealth.com')
      THEN true
      ELSE false
    END
  );
...
-- Final check: Ensure service_role can bypass RLS (this is default but let's be explicit)
...
DO $$
BEGIN
  RAISE NOTICE '✅ Phase 2 migration complete! All tables, triggers, and RLS policies have been set up.';
  RAISE NOTICE '📝 Next step: Sign in to the application to test user creation.';
END $$;
